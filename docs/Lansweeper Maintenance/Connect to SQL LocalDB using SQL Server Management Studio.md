...
2. Submit the server name. For SQL LocalDB the server name should always be "(localdb)\.\LSInstance". 
3. Select your authentication type. You can use Windows Authentication or SQL Server Authentication.
   - Windows Authentication  
     The user account you are logged on as is automatically selected when you select Windows Authentication. Make sure to log on with the user account that last installed or updated Lansweeper as this is the SA account to your SQL LocalDB instance. Only the SA account can connect to the LocalDB instance.    
