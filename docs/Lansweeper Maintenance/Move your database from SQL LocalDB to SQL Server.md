...
4. Configure the installation exactly as shown below. This will install a default Lansweeper database in the SQL Server instance specified by you.   
...
7. In the **Files** tab select **Relocate all files to folder** and check **Overwrite the existing database** in the Options tab. Select **OK** to perform the restore.   
...
9. Submit the SQL instance name and database password in ConfigEditor.
   - Run the ConfigEditor tool, found at `Program Files (x86)\Lansweeper\Tools\ConfigEditor.exe` on the servers hosting your Lansweeper Server service and web console. 
   ...
   - Select the Password field, select **Edit** and submit the same password you previously used in the database script.****
   ...
   - Select **Save configs and restart service**. You've now successfully migrated your SQL LocalDB database to SQL Server. 
