...
Active Directory User/Group Path requires a credential with read-only access to Active Directory. You can either use a credential with the necessary privileges as your global Windows credential or map the credential to the domain you are trying to scan. More info on submitting and mapping scanning credentials can be found in [this knowledge base article](/classic/docs/create-and-map-scanning-credentials).
