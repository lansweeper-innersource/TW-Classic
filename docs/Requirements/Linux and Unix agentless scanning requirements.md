...
6. Verify whether SSH is running by entering the command below in the console. If there's output, SSH is running; if there's no output, SSH is not running.  
   `ps -e | grep sshd`
7. You now meet the requirements for agentless Linux and Unix scanning. Start scanning the computer by following the instructions in [this knowledge base article](/docs/how-to-scan-a-linux-or-unix-computer).
