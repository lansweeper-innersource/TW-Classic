...
- If the machine hosting your Lansweeper installation is not running Windows Server 2008 R2 and the scanning errors are limited to specific Windows computers, it is also possible that WMI (Windows Management Instrumentation) is corrupt on those client machines. More info on resolving WMI corruption errors can be found in [this knowledge base article](/classic/docs/repair-a-corrupt-wmi-installation).
