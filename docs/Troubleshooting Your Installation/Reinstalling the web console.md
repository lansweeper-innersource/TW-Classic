...
4. If you added any documents, images or widgets to Lansweeper, create a backup of these files. Information on which website subfolders store which data can be found in [this knowledge base article](/classic/docs/where-are-lansweeper-data-reports-and-settings-stored#heading2 "Where lansweeper data reports and settings are stored").  
...
11. Select the web server you'd like to install the console under. Selecting **Recommended** will install the web console under IIS Express, while selecting **Advanced** will install the web console under IIS. The Advanced option will only be available if IIS is already enabled on your computer. Instructions for enabling IIS can be found in [this knowledge base article](/classic/docs/install-iis-internet-information-services).   
